...
const Expertise = () => {
    ...
    return (
        <section id="expertise" className="py-12 md:py-16 lg:py-20 bg-transparent relative" aria-label="Core Expertise">
            <div className="container mx-auto px-6 md:px-12 max-w-7xl">
                ...
                <Tabs value={selected} onValueChange={(v) => setSelected(v)} className="w-full">
                        ...
                        {["clinical", "sap", "cdisc", "therapeutic"].map((tab, idx) => {
                            ...
                            return (
                                <button
                                    ...
                                    aria-label={`${label} expertise tab`}
                    ...
                    <div className="hidden md:block mb-4 md:mb-6 lg:mb-8">
                        <TabsList className="w-full max-w-3xl mx-auto flex justify-center bg-slate-100 p-1 rounded-full border border-slate-200" role="tablist">
    ...
    return (
        ...
        >
            ...
            <article className="space-y-6">
                ...
                <ul className="space-y-3 pl-1" role="list">
                    {bullets.map((item, idx) => (
                        <li key={idx} className="flex items-start group" role="listitem">
                            <span className="w-1.5 h-1.5 bg-slate-400 rounded-full mt-2.5 mr-4 group-hover:bg-slate-600 transition-colors duration-200" aria-hidden="true" />
            ...
            >
                <img
                    src={imageSrc}
                    alt={`${title} - Professional illustration showing expertise in ${title.toLowerCase()}`}
                    className="w-full h-full object-cover object-center"
                    loading="lazy"
                    decoding="async"
                />
                <figcaption className="sr-only">{title} expertise illustration</figcaption>
            </motion.figure>
