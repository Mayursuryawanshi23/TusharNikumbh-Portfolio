...
const Experience = () => {
    return (
        <section id="experience" className="py-24 relative overflow-hidden" aria-label="Professional Experience Timeline">
            ...
            <div className="container mx-auto px-6 md:px-12 max-w-5xl relative z-10">
                ...
                >
                    ...
                    <div className="h-1 w-20 bg-white mt-6 rounded-full mx-auto md:mx-0" aria-hidden="true"></div>
                </motion.div>

                <ol className="relative border-l border-white ml-3 md:ml-6 space-y-12" role="list">
...
const ExperienceCard = ({ data, index }) => {
    ...
    return (
        ...
        >
            {/* Timeline Node */}
            <div 
                className={`absolute left-[-5px] md:left-[-6px] top-2 w-3 h-3 rounded-full border-2 transition-colors duration-200 ${index === 0 ? 'bg-white border-white' : 'bg-white border-white group-hover:border-white'}`}
                aria-hidden="true"
            />
            ...
            <article
                ...
                aria-label={`${data.title} at ${data.company}, ${data.period}`}
            ...
            >
                <div className="flex flex-col md:flex-row md:items-baseline md:justify-between mb-2">
                    ...
                    <time className="text-xs font-bold uppercase tracking-wider text-white group-hover/card:text-white mt-1 md:mt-0 transition-colors duration-200" dateTime={data.period}>
                        {data.period}
                    </time>
                </div>
                ...
                <AnimatePresence>
                    {isOpen && (
                        ...
                        >
                            <ul className="mt-6 space-y-3 border-t border-white group-hover/card:border-white pt-6 transition-colors duration-200" role="list">
                                {data.details.map((detail, idx) => (
                                    <li key={idx} className="flex items-start text-white group-hover/card:text-white text-sm md:text-base font-light transition-colors duration-200" role="listitem">
                                        <span className="w-1.5 h-1.5 bg-white group-hover/card:bg-white rounded-full mt-2 mr-3 flex-shrink-0 transition-colors duration-200" aria-hidden="true" />
                                        {detail}
                                    </li>
                                ))}
                            </ul>
